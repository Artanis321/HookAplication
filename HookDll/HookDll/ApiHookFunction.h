...
int writeFile(DWORD processID, string originalFuncion) {

	time_t now = time(NULL);
	tm* ltm = localtime(&now);
	ofstream myFile;
	myFile.open("C:\\Hook\\hookAplicationResult.txt", ofstream::app);
	myFile << 1 + ltm->tm_hour << ":" << 1 + ltm->tm_min << ":" << 1 + ltm->tm_sec << ";" + std::to_string(processID) + ";" + originalFuncion + ";" << endl;
	myFile.close();
	return 0;
